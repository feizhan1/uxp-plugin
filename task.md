...
## ✅ 删除SKU和场景图区域的局部勾选删除按钮 (2026-02-05)

### 完成情况：删除局部勾选删除按钮，保留顶部全局勾选删除功能

**需求背景**：
- SKU和场景图区域存在局部"勾选删除"按钮和"一键删除"按钮，功能重复
- 顶部已有全局勾选删除功能，可以完成所有局部勾选删除的工作
- 简化UI，减少按钮数量，提升用户体验

**实现方案**：

#### 1. 删除SKU区域的勾选删除按钮组 (src/components/ProductDetail.jsx:5196-5233)
- ✅ 删除勾选删除模式的控制按钮（取消、全选、确定）
- ✅ 删除进入勾选删除模式的按钮
- ✅ 保留"一键删除"按钮

#### 2. 删除场景图区域的勾选删除按钮组 (src/components/ProductDetail.jsx:5338-5375)
- ✅ 删除勾选删除模式的控制按钮（取消、全选、确定）
- ✅ 删除进入勾选删除模式的按钮
- ✅ 保留"一键删除"按钮

#### 3. 修改SKU图片的checkbox显示条件 (src/components/ProductDetail.jsx:5231-5233)
- ✅ 删除局部勾选删除模式的条件 `selectDeleteMode.type === 'sku'`
- ✅ 保留批量同步模式的条件 `batchSyncMode && skuIndex === 0`
- ✅ 保留全局勾选删除模式的条件 `selectDeleteMode.type === 'global'`

#### 4. 修改场景图片的checkbox显示条件 (src/components/ProductDetail.jsx:5371)
- ✅ 删除局部勾选删除模式的条件 `selectDeleteMode.type === 'scene'`
- ✅ 保留全局勾选删除模式的条件 `selectDeleteMode.type === 'global'`

#### 5. 修改单张删除按钮的显示条件
- ✅ SKU图片：修改为非批量同步模式且非全局勾选删除模式时显示
- ✅ 场景图片：修改为非全局勾选删除模式时显示

**保留功能**：
- ✅ 顶部全局勾选删除功能（`selectDeleteMode.type === 'global'`）
- ✅ `selectedImages` 状态（批量同步功能需要使用）
- ✅ 所有相关处理函数（全局勾选删除需要使用）
- ✅ 批量同步功能的checkbox显示逻辑

**测试验证**：
- ✅ 构建成功，无语法错误
- ⏳ 需要手动测试顶部全局勾选删除功能是否正常工作
- ⏳ 需要手动测试批量同步功能是否正常工作

**Git提交**：commit 0d37550
...
**问题描述**：
- 产品列表标题硬编码为"待处理产品列表"，不随状态选择变化
- 显示的数量为所有产品总数，而非当前状态下过滤后的数量
- 用户无法直观了解当前查看的是哪个状态的产品列表

**需求**：
- 根据 statusFilter 的值动态显示标题：
  - 状态3（待处理）：显示"待处理产品列表"
  - 状态4（编辑审核中）：显示"编辑审核中产品列表"
- 显示数量为过滤后当前状态的产品数量

**技术实现**：

#### TodoList.jsx 修改标题显示逻辑 (src/panels/TodoList.jsx:1516-1519)
...
## ✅ 移除SKU图片20张显示限制 (2025-10-30)

### 完成情况：移除硬编码的20张SKU图片显示限制，支持显示所有图片

**问题描述**：
- ProductDetail.jsx 中存在硬编码的20张SKU图片显示限制
- 用户索引文件中有28张SKU图片，但UI只能显示前20张
- 导致部分图片无法在界面中查看和操作

**技术实现**：

#### ProductDetail.jsx 修改虚拟化逻辑 (src/components/ProductDetail.jsx:425)
...
**修复效果**：
- 完全移除SKU图片数量显示限制
- 所有SKU图片（包括28张或更多）都能正常显示
- 保持与原始图片和场景图片的显示逻辑一致性
